...
pub fn max_dimensions(dims: &Vec<(u32, u32)>) -> (u32, u32) {
    if dims.len() == 0 {
...
pub fn swizzle_4(images: (&GrayImage, &GrayImage, &GrayImage, &GrayImage)) -> RgbaImage {
    let max_dims = max_dimensions(&[images.0, images.1, images.2, images.3].iter().map(|i| i.dimensions()).collect());
    ...
    let result = RgbaImage::from_fn(max_dims.0, max_dims.1, |x, y| {
        ...
        Rgba([
            imageops::sample_nearest(images.0, u, v).expect("uv in bounds").0[0],
            imageops::sample_nearest(images.1, u, v).expect("uv in bounds").0[0],
            imageops::sample_nearest(images.2, u, v).expect("uv in bounds").0[0],
            imageops::sample_nearest(images.3, u, v).expect("uv in bounds").0[0]
        ])
    });
...
pub fn _swizzle_3(images: (&GrayImage, &GrayImage, &GrayImage)) -> RgbImage {
    let max_dims = max_dimensions(&[images.0, images.1, images.2].iter().map(|i| i.dimensions()).collect());

    let result = RgbImage::from_fn(max_dims.0, max_dims.1, |x, y| {
        let u = x as f32 / max_dims.0 as f32;
        let v = y as f32 / max_dims.1 as f32;
...
impl Default for NormalMap {
    fn default() -> Self {
        let default = RgbImage::from_fn(256, 256, |_,_| {
            Rgb([128, 128, 255])
        });
...
impl BasicPBR {
    pub fn new(metallic: &MetallicMap, roughness: &RoughnessMap, ao: &AoMap, emissive: &EmissiveMap) -> Self {
...
impl AdvancedPBR {
    pub fn new(normal: &NormalMap, ao: &AoMap, roughness: &RoughnessMap) -> Self {
        let max_dims = max_dimensions(&vec![normal.0.dimensions(), ao.dimensions(), roughness.dimensions()]);

        let result = RgbaImage::from_fn(max_dims.0, max_dims.1, |x, y| {
            let u = x as f32 / max_dims.0 as f32;
            let v = y as f32 / max_dims.1 as f32;

            let normal_sample = imageops::sample_nearest(&normal.0, u, v).expect("uv in bounds").0;

            Rgba([
                normal_sample[0],
                normal_sample[1],
                imageops::sample_nearest(&ao.0, u, v).expect("uv in bounds").0[0],
                imageops::sample_nearest(&roughness.0, u, v).expect("uv in bounds").0[0]
            ])
        });
...
mod test {
    ...
    pub fn test_basic_pbr() -> Result<(), Box<dyn Error>> {
        ...
        let metallic = ImageReader::open(test_folder.join("HELMM.png"))?.decode()?.into_luma8();
        let metallic = MetallicMap(metallic);
        let roughness = ImageReader::open(test_folder.join("HELMR.png"))?.decode()?.into_luma8();
        let roughness = RoughnessMap(roughness);
        let ao = ImageReader::open(test_folder.join("HELMAO.png"))?.decode()?.into_luma8();
        let ao = AoMap(ao);
        let emissive = default_pbr_channel(255);
        let emissive = EmissiveMap(emissive);

        let pbr = BasicPBR::new(&metallic, &roughness, &ao, &emissive);
        let mut out_file = OpenOptions::new().create(true).write(true).open(out_folder.join("basic_pbr.png"))?;
    ...
    pub fn test_advanced() -> Result<(), Box<dyn Error>> {
        ...
        let out_folder = test_folder.join("out");
        create_dir_all(&out_folder)?;
        let normal = NormalMap(ImageReader::open(test_folder.join("HELMN.png"))?.decode()?.into_rgb8());
        ...
        let mut out_file = OpenOptions::new().create(true).write(true).open(out_folder.join("advanced_pbr.png"))?;
        let png_encoder = PngEncoder::new(&mut out_file);
        pbr.write_with_encoder(png_encoder)?;
